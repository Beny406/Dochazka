...
public class FXMLDochazkaDovolenaController implements Initializable {
    ...
    List<LocalDate> dovolena = new ArrayList<>();
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        checkList.setCellFactory(new Callback<ListView<CheckListDen>, ListCell<CheckListDen>>(){
            ...
            public ListCell<CheckListDen> call(ListView<CheckListDen> param) {
                return new CheckBoxListCell<CheckListDen>(CheckListDen::onProperty){
                    @Override 
                    public void updateItem(CheckListDen item, boolean empty){
                        super.updateItem(item, empty);
                        if (item != null){
                            setDisable(false);
                            item.onProperty().addListener((obs, oldValue, newValue) -> {
                                if (newValue) {
                                    checkList.getCheckModel().check(item);
                                }
                                else {
                                    checkList.getCheckModel().clearCheck(item);
                                }
                            });
                            ...
                            if (dovolena.stream().anyMatch(denDovolene -> denDovolene.equals(item.getLocalDate()))){
                                item.setOn(true);
                            }
                        }
                    }
