...
public class FXMLDochazkaParagrafController implements Initializable {

    ObservableList<CheckListDen> data = FXCollections.observableArrayList();
    List<LocalDate> paragraf = new ArrayList<>();
                    ...
                    @Override public void updateItem(CheckListDen item, boolean empty){
                        ...
                        if (item != null){
                            ...
                            item.onProperty().addListener((obs, oldValue, newValue) -> {
                                ...
                                if (paragraf.stream().anyMatch(denDovolene -> denDovolene.equals(item.getLocalDate()))){
                                    item.setOn(true);
                                }
                        }
                    }
